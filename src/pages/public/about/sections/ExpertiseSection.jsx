        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-9 md:gap-12">
          {Object.entries(SERVICES).map(([key, service]) => (
            <div key={key} className="service-col flex flex-col gap-5">
              <h3 className="font-sequel font-black italic text-[clamp(1.2rem,2vw,1.8rem)] tracking-tight uppercase text-black leading-none">
                {service.title}
              </h3>
              <p className="font-sans text-[13px] leading-[1.75] text-[#666]">
                {service.description}
              </p>
              <ul className="flex flex-col gap-1.5 mt-2">
                {service.items.map((item) => (
                  <li
                    key={item}
                    className="font-sequel font-extrabold italic text-[11px] text-black uppercase tracking-wide py-0.5"
                  >
                    {item}
                  </li>
                ))}
